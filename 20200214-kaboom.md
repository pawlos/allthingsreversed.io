...
This wasn't a difficult task but it had one tricky trick that made it take way more time than it should be required.
...
It's clear what is expected from us here. We should pass an argument that's `defuse` and if that the case we pass a `INS{` prefix and some YouTube link to `checkFlag` function, along with the length of the `INS` string. So the logic for the flag must be hidden in the `checkFlag` function. The function is long (~900 lines) but investigating it for extended period of time did not reveal any interesting part. It looked like an over-engineered version of `strcmp`. And if we look at arguments passed to this function - one being `INS` and the other a YT-link that can never be equal - or can they? And if not where's the flag.

The answer is the innocent, overlooked UPX. That sneaky little part was modified so it not only unpacks the compressed data and passes the execution to it. It does a little bit more.
...
it will move some amount of bytes (`0x11a1`) around! And that my friends where the true flag will be revealed. As in this case after unpacking we will no longer have a YT link where we previously had it but a correct flag.
