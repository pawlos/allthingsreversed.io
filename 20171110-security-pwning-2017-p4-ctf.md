...
This year I did a bit better than the last time - 8th position (the last one with the prize) but could be better as at some point I was on the 5th. Here are solutions to the tasks I've managed to solve.
...
Assuming the key is long, I took an educated guess it will be 23 char and with providing an input for the most occuring char being the space we get the flag:
...
This one was one of the most interesting one (from those I did of course). What we asked to do was to pay $ 12300 and we will get the flag. Of course no one would pay that much to get the flag so we need to trick the service into thinking we actually did pay that amount. The web page had only the standard "Pay" button.
Upon inspection of the HTML we notice all the bits and pieces that are required for PayPal are there including the URL that will listen to PayPal notification of the successful transaction. Maybe it's not written properly and we can forge one.
...
This was a simple Windows RE. When run it shows nice ASCII art which is also used for displaying 'No', 'Yes' message so you can't find them (easily) in the resources.

The algo itself is simple, it xores first char, with the next one and so on. You can get it from the code and it's also confirmed by the unencrypted last char for the encrypted password found in the binary.
...
It could be solvable with symbolic execution (but I'm not skilled there yet) so I went with the manual approach. There's not that many letters and they are easily retrievable so I did not spend that much time on it and got the flag: `pwn{recovering_flag_one_byte_at_a_time}`.

# Uber keygen (re 100)

This one looks like a big brother of the previous one. There are far more checks, too many to do the manual recovery and if we look closer it looks like this time we are retrieving the flag bit by bit. We need some automated way then. Again it would be perfect for symbolic execution but I went with radare2 (namely r2pipe).
...
In this one we were given a [text file](https://pwning2017.p4.team/downloads/vuln.ll_ff667d25b3a2db3c391da9ea65e132ab6b6adc86.txt) with an "unknown" format. It looks like some kind of Asm and upon further inspections it's probably generated by clang.

> clang version 6.0.0

My first impression was to to understand the format completely and I started to comment the file but after a short while I've noticed a repeated pattern like this:
...
# Military grade algorithm (re 150)

The last RE challange was similar to the previous one but here each char is checked with a separate `check_flag_x` function. This already gives us the information about the length but makes a bit more difficult if we want to take the same approach as with "Uber keygen". At first I've tried that but the functions are a bit more complex and again it would be perfect to do the symbolic executions here. I did something else. I've rewritten some of the `check_flag_x` in python and run them to get the flag char.

When I run it (not even fully implemented all the methods) I got some gibberish on one or two chars (and it looks like there is some error i.e. char 11) but it was correct on so many chars that I could retrieve the flag:
