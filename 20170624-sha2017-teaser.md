...
We've jumped to the second country. During the CTF I've took and approach to convert Country name to IP address and for the first attempt I've tried [ip2location.com](https://lite.ip2location.com/). It has a nice URL that I could create easily by reading the country from the source. But with some checking it turned out that the IPs were detected as wrong so I've needed another IP db.

I've found one at <http://www.nirsoft.net/>. I've missed the info on the page that I have the two-letter code of the country and I've took a longer approach by finding the link to the page by Country name and then accessing it to find IP.
