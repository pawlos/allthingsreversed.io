...
but there was a problem with such approach as it still took too long to extract a file. I suspect each time `peepdf` was loading a file it was analyzing it and that was taking the precious seconds. I started working on an in memory extracted to speed things up another team member extracted the last матрёшка-file with a `binwalk` script using `-M` option. I didn't even know that `binwalk` can do it. Nice...
...
But maybe we can somehow gain the knowledge about the flag? For sure we can. The problem here is that data is compressed before being encrypted and the algorithm that is used is [deflate](https://tools.ietf.org/html/rfc1951). Why is that bad? Well if we can match data that we pass through the `user` parameter with the actual flag, we can influence `deflate` algorithm. It will detect the repeated data in the string and the result will be shorter than if there would be no repetition. We can use this behavior to obtain the flag. By brute-forcing the flag char-by-char we can get it.
