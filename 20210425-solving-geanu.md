...
The task starts by printing the following ASCII-art of Keanu and awaits our input.

![](content/images/2021/04/keanu_from_geanu-1.png)Keanu from Geanu ;)

After getting user input it finishes.

Let's fire up Ghidra and load the binary. Since the binary is written in Go, we can apply 'golang_renamer.py` to demangle function names, but it's not really needed to solve this challenge.

We could start from main ( `main_main` in golang) and see an interesting comparison there

![](content/images/2021/04/image.png)Right in the main, we see an interesting comparison.
...
Also, if you have been paying attention, we are in the same method that is guarded by the `if` statement from `main`. So if we add 2 from analysis of the `main`, and 2 from the analysis of what this method does, for sure we should pay a bit more closer attention to it.
